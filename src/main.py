...
while (simulationRunning):
    ...
    logBootOrder()
    for i, x in enumerate(bootOrder):
    ...
    simulationsRan += 1
    simsLeft -= 1
    if (AUTORUN and simsLeft == 0):
        simulationRunning = False
    elif not AUTORUN:
        keepRunning = input("Press enter to proceed, or \"QUIT\" to quit.")
        if keepRunning == "QUIT":
            simulationRunning = False


print(f"Simulations ran: {simulationsRan}")
for x in winners:
    print(x)
